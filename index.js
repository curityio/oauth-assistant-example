...
const loginFlowWithPopupButton = document.querySelector("#loginFlowWithPopup");
const loginFlowWithPopupMessage= document.querySelector("#loginFlowWithPopupMsg");

const loginFlowHiddenFallbackToPopupButton = document.querySelector("#loginFlowHiddenFallbackToPopup");
const loginFlowHiddenFallbackToPopupMessage= document.querySelector("#loginFlowHiddenFallbackToPopupMsg");
...
const redirectUriForPopup = window.origin + "/assisted.html";
...
function addEventListeners() {
    ...
    loginFlowWithPopupButton.addEventListener('click', (evt) => {
        console.log("Start Login With Visible Popup = true");

        assistantForPopup().then(() => assistant.authorizePopup(JSON.parse(authorizeParamsInput.value))
          .then((token) => {
              console.log("Got token: " + token);
              loginFlowWithPopupMessage.innerHTML = "Token: " + token;
          })
          .catch((err) => {
              if (err.error === "login_required") {
                  loginFlowWithPopupMessage.innerHTML = "Login Required";
              } else {
                  loginFlowWithPopupMessage.innerHTML = getFormattedError(err);
              }
          })
        );
    });
    ...
    loginFlowHiddenFallbackToPopupButton.addEventListener('click', (evt) => {
        console.log("Start Login With Hidden Flow Fallback to Popup = true");

        assistantForIframe().then(() => assistant.authorizeHiddenFallbackToPopup(JSON.parse(authorizeParamsInput.value))
          .then((token) => {
              console.log("Got token: " + token);
              loginFlowHiddenFallbackToPopupMessage.innerHTML = "Token: " + token;
          })
          .catch((err) => {
              if (err.error === "login_required") {
                  loginFlowHiddenFallbackToPopupMessage.innerHTML = "Login Required";
              } else {
                  loginFlowHiddenFallbackToPopupMessage.innerHTML = getFormattedError(err);
              }
          })
        );
    });
...
function assistantForPopup() {
    return initAssistantWithRedirectUri(redirectUriForPopup);
}
...
let authorizeSettings = defaultAuthorizeSettings;
if (localStorage.getItem('authorizeSettings')) {
    try {
        authorizeSettings = JSON.parse(localStorage.getItem('authorizeSettings'));
    } catch(e) {
    }
}
...
authorizeSettingsInput.addEventListener("change", () => {
    const settings = JSON.parse(authorizeSettingsInput.value);
    localStorage.setItem('authorizeSettings', JSON.stringify(settings));
    init(settings);
});
